...
def main():
    ...
    # read
    _,exp,ens = tag.split('.')
    ds = open_ds_3h_atm(exp, ens, chunks={})
    ds = ds.sel(time=TSLICES[exp])

    # compute monthly absolute 250 hPa wind speed
    F = np.sqrt(ds.U250**2+ds.V250**2)
    F = F.rename(NAME)
    Fmm = F.resample(time='MS').mean().compute()

    # take interannual mean
    print('groupby month')
    Fam = Fmm.groupby('time.month').mean()
    out = Fam.to_dataset()
