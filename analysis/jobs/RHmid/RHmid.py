...
def main():
    ...
    # get pressure-weighted mean relative humidity from 400 to 700 hPa
    pi = interpolate.pressure_from_hybrid(ds, cell_interface=True)
    RHmid = interpolate.integrate(ds.RELHUM, pi, 40000, 70000) / (70000-40000)
    RHmid = RHmid.rename(NAME)

    # take interannual mean 
    RHmidmean = RHmid.groupby('time.month').mean()
    out = RHmidmean.to_dataset()
