...
N_extend = 5 # number of steps to extend valid tracks using optional RV maxima (default 0, use -1 for unlimited)
...
#-----------------------------------------------------------------------------------------
        for track_i in track_ID_active.copy(): # loop over active tracks (from previous data chunk)
            exec(f'track = TRACK_ID_{track_i}')
            
            if track.ndim == 1:
                continue

            # continue to next track if maximum number of extensions has been reached
            # there is no such check for negative N_extend, i.e. tracks will continue until no new points are found
            if (N_extend > 0) and (np.any(track[:,9]<0)):
                time_f = np.nonzero(track[:,9]<0)[0][0] # time index of first weakening
            ...
            if np.any(distance <= 200) == True:  # Match with regular RV maximum
                ...
                # previous value is optional -> end of track
                if (vor_850_TC_prev < 0) and (N_extend == 0):  # Negative RV850 implies the previous point was optional -> end of track
                    # do no update TC track and remove previous point
                    print(f'end TRACK_ID_{track_i} (second weakening)')
                    exec(f'TRACK_ID_{track_i} = TRACK_ID_{track_i}[:-1]')
                    continue
                ...
                # previous value is optional but extension requested -> validate track now
                ...
                distance = distance_adjacent_RV_maxima(PSL_min_lon_opt, PSL_min_lat_opt, lon, lat, track)
                if not np.any(distance <= 200): # no match with new (optional) RV maximum
                    continue
                index_min   = np.argmin(distance)

                # set weakening flag (i.e. negative RVmax value)
                # since we're appending optional maxima, its generally always true
                # however, when running an extension, only the first time is flagged
                weakening = True if (N_extend == 0) or (np.sum(track[:,9]<0) < 1) else False

                # append data
                data_track,_ = fill_track_data(PSL_min_lon_opt, PSL_min_lat_opt, index_min, lon, lat, lat_weight, lon_ocn, lat_ocn, temp_anom_opt, time_i,
                    pres, pres_month, vor_850, U_10, SST_day, SST_month, u_vel_850, v_vel_850, u_vel_250, v_vel_250, weakening=weakening)
                exec(f'TRACK_ID_{track_i} = np.ma.vstack([TRACK_ID_{track_i}, data_track])')
