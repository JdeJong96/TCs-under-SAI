...
if (experiment == 'ref') and (run_ensemble<=5):
    ...
    time_slice      = ('2003-01-01','2004-01-01') # time period to analyse
    ...
    RVSEARCHRADIUS  = 200 # (km) search radius for TC at previous time step
    NOUT            = 8 # number of time steps per day
    time_slice      = ('2093-01-01','2094-01-01') # time period to analyse
    outfile         = f'TC_tracker_results.{experiment}.started_{run_year_start}.{run_ensemble:03d}.nc'
...
for task in tasks:
    ...
    for time_i in range(len(time)):
        ...
        for track_i in track_ID_active: # loop over active tracks (from previous data chunk)
            ...
            if np.any(distance <= 200) == True:
                ...
                if vor_850_TC_prev < 0:
                    ...
                    print(f'TRACK_ID_{track_i}[-1,9] += 1 (remove weakening flag)')
                ...
                #Get the correct ID
                print(f'app TRACK_ID_{track_i} = {data_track[:3]}')
                ...
                if vor_850_TC_prev < 0:
                    ...
                    print(f'TRACK_ID_{track_i} = TRACK_ID_{track_i}[:-1] (second weakening)')
                    ...
                    if np.any(distance <= 200) == True:
                        ...
                        #Get the correct ID
                        print(f'app TRACK_ID_{track_i} = {data_track[:3]} (first weakening))')
                        exec('TRACK_ID_'+str(track_i)+' = np.ma.vstack([TRACK_ID_'+str(track_i)+', data_track])')
        ...
        for min_i in range(len(PSL_min_lat)):
            ...
            print(f'ini TRACK_ID_{track_ID} = {data_track[:3]}')
            ...
            if duration_track >= 48.0:
                ...
                if time_j != 0:
                    ...
                    if len(track)  * 3.0 < 48:
                        ...
                        print(f"del TRACK_ID_{track_i} (tooshortland, set origin lat to 90N)")
                ...
                if np.abs(track[0,2]) > 30.0:
                    print(f'del TRACK_ID_{track_i} (origin)')
                if vel_max_counter != 8:
                    print(f'del TRACK_ID_{track_i} (velmax)')
                if shear_counter != 8:
                    print(f'del TRACK_ID_{track_i} (shear)')
                if track[0,7] < 25.0:
                    print(f'del TRACK_ID_{track_i} (SST)')
                ...
                track_ID_active.remove(track_i)
                print(f'del TRACK_ID_{track_i} (tooshortregular)')
