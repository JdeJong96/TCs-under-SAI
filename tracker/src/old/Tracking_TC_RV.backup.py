...
if experiment == 'rcp':
	...
	datadir			= {'atm': ddir+'/atm/hist/3h', 
					   ...
					   'ocn':ddir+'/ocn/msf/monthly' if run_year_start==2002 else ''}
	print('warning: '+'using monthly ocean data' if run_year_start==2002 else 'not using ocean data')
...
def ReadinData(filenames, x_diff, y_diff, t1, t2):
	...
	with netcdf.MFDataset(filenames, 'r', aggdim='time') as fh:
		...
		if track_attrs[0] == '':
			...
			track_attrs[9]	= f"RV:{{'units':{fh['U850'].units}/m}}"
			track_attrs[10]	= f"Vshear:{{'units':'{fh['U850'].units}'}}"
...
def ReadinDataSST(month, year):
	...
	try:
		...
		if track_attrs[7] == '':
			...
			track_attrs[8] = "SSTmon:{'units':{'"+fh['SST'].units+"'}"
		fh.close()
...
#	time_RV		= cftime.num2date(time_RV[:], time_RV.units, getattr(time_RV, 'calendar', 'noleap'))
...
def RadiusMaximumWind(lon, lat, lon_index, lat_index, vel_field_all, reduced_radius = False):
	...
	if track_attrs[6] == '':
		track_attrs[6]	= "rad_vel_max:{'units':'km'}"
...
for task in tasks:
	...
	for time_i in range(len(time)):
		...
		try:
#			PSL_min_lon, PSL_min_lat, temp_anom = ReadinData_RV_Max(directory_output, timestamp[time_i])
			PSL_min_lon, PSL_min_lat, temp_anom = ReadinData_RV_Max(RVfile, timestamp[time_i])
#			print(f"There are {len(PSL_min_lon)} vorticity maxima in RVfile at {time_i=}")
		except FileNotFoundError:
			continue
		...
		if time_year == 2008 or time_year == 2098:
			print(f"Reached year {time_year}, aborting...")
			break
...
#				if distance[min_i] <= 200.0:
#					print(f"l709 (tracks > 2) {track_i=} {min_i=}: ({lat_1:.1f},{lon_1:.1f}) - ({lat_2:.1f},{lon_2:.1f}) = {distance[min_i]:.3f} km")
			...
			if np.any(distance <= 200) == True:
				...
				#Get the correct ID
#				print('line 733: TRACK_ID_'+str(track_i)+' = np.ma.vstack([TRACK_ID_'+str(track_i)+', data_track])')
...
#						if distance[min_i] <= 200.0:
#							print(f"l808 (tracks > 2 opt) {track_i=} {min_i=}: ({lat_1:.1f},{lon_1:.1f}) - ({lat_2:.1f},{lon_2:.1f}) = {distance[min_i]:.3f} km")
					...
					if np.any(distance <= 200) == True:
						...
						#Get the correct ID
#						print('line 817: TRACK_ID_'+str(track_i)+' = np.ma.vstack([TRACK_ID_'+str(track_i)+', data_track])')
						exec('TRACK_ID_'+str(track_i)+' = np.ma.vstack([TRACK_ID_'+str(track_i)+', data_track])')
		...
		for track_i in track_ID_active: # 
			...
			for min_i in range(len(PSL_min_lat)):
				...
				distance[min_i] = Distance(lon_1, lat_1, lon_2, lat_2) / 1000.0
#				if distance[min_i] <= 200.0:
#					print(f"l898 (tracks 1) {track_i=} {min_i=}: ({lat_1:.1f},{lon_1:.1f}) - ({lat_2:.1f},{lon_2:.1f}) = {distance[min_i]:.3f} km")
			...
			if np.any(distance <= 200) == True:
				...
				#Get the correct ID
#				print('line 906: TRACK_ID_'+str(track_i)+' = np.ma.vstack([TRACK_ID_'+str(track_i)+', data_track])')
				exec('TRACK_ID_'+str(track_i)+' = np.ma.vstack([TRACK_ID_'+str(track_i)+', data_track])')
...
#		print(f"{len(remove_index)} maxima assigned to short tracks. Remaining {len(PSL_min_lon)} valid maxima will start new tracks.")
...
# HEAT_data = netcdf.Dataset(directory_output+'TC_tracker_RV.test.nc', 'w')
...
HEAT_data.variables['data'].longname	= 'Time, lon (pres), lat (pres), pres (3-h), pres (month), max vel, rad. max vel, SST (day), SST (month), RV max (1/s), wind shear (m/s), prec max (m/s)'
HEAT_data.variables['data'].attstring	= track_attrs 
