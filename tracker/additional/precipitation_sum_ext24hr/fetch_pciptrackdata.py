...
def extend_24hr(da, track, mask, result):
    """extend track precipitation data by 24 hours at last track position"""
    track = track.where(track.notnull(), drop=True)
    dt = datetime.timedelta(hours=3)
    ext_time = track.cftime.data[-1] + np.arange(1,9)*dt
    ext_track = track.isel(dtime=-1).assign_coords(cftime=ext_time)
    da = da.interp(time=ext_track.cftime.data).load()
    dx = int((mask.shape[1]-1)/2)
    dy = int((mask.shape[0]-1)/2)
    x = np.searchsorted(da.lon, ext_track.lon)
    y = np.searchsorted(da.lat, ext_track.lat)
    xs = np.arange(x-dx,x+dx+1,)%len(da.lon)
    ys = np.arange(max(y-dy,0),min(y+dy+1,len(da.lat)))
    for t,_ in enumerate(da.time):
        result[ys,xs] += da.isel(time=t)[ys,xs] * mask * 3
...
def main():
    ...
    for year in years[:]:
        ...
        for tcid in dsy.id[:]:
            ...
            extend_24hr(pcip, track, mask, result)
            #t2 = perf_counter()
